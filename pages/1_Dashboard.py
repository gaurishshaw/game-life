...
st.title(f"{cls_def.get('emoji','⚔️')} Sanctum")
...
col2.metric("Protocols Today", f"{completed_today} / {len(due_today)}")
col3.metric("Overdue Quests", len(overdue), delta=f"-{len(overdue)}" if overdue else None,
            delta_color="inverse")
...
with col_left:
    st.subheader("📅 Today's Protocols")
    if not due_today:
...
with col_right:
    st.subheader("📜 Overdue Questlines")
    if not overdue:
