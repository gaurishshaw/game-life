...
def create_todo(username: str, title: str, notes: str, difficulty: str,
                due_date: str = None, priority_flag: int = 0) -> int:
    conn = get_connection()
    with conn:
        cur = conn.execute("""
            INSERT INTO todos (username, title, notes, difficulty, due_date, priority_flag)
            VALUES (?,?,?,?,?,?)
        """, (username, title, notes, difficulty, due_date, priority_flag))
    conn.close()
    return cur.lastrowid
